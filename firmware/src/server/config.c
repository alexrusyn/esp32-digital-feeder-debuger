...
static ip_ready_cb_t userReadyCb = NULL;
...
static void wifiConnectHandler(void *arg, esp_event_base_t event_base, int32_t event_id, void *event_data)
{
    if (event_base == IP_EVENT && event_id == IP_EVENT_STA_GOT_IP)
    {
        ...
        if (httpd_start(&server, &config) == ESP_OK && userReadyCb)
        {
            userReadyCb(server);
        }
    }
...
void initWifi(ip_ready_cb_t userCb)
{
    userReadyCb = userCb;
    ...
    ESP_ERROR_CHECK(esp_netif_set_ip_info(staNetif, &ip_info));
